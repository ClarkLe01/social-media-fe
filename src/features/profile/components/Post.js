...
function Post() {
    ...
    const {
        ...
        UserPostListFetchNextPage,
    } = useUserPost(userId); // posts of current user
    ...
    useEffect(() => {
        if (!UserPostListLoading && UserPostList) {
            setUserPosts(
                UserPostList.pages.reduce((acc, cur) => {
    ...
    useEffect(() => {
        const onScroll = () => {
            handleScroll({ UserPostListFetchNextPage, UserPostListHasNextPage, UserPostListFetching });
        };

        document.addEventListener('scroll', onScroll);

        return () => removeEventListener('scroll', onScroll);
    }, [ UserPostListHasNextPage, UserPostListFetchNextPage, UserPostListFetching ]);
                ...
                {user && (
                    <div className="card w-100 shadow-xss rounded-xxl border-0 px-4 pt-4 pb-2 mb-3">
                        ...
                        <div className="card-body p-0 py-3 d-flex justify-content-center">
                            <iframe
                                src="https://embed.lottiefiles.com/animation/114386"
                                width={'100%'}
                            />
                        </div>
                    </div>
